      ...
      <table>
        <tr>
          <td
            ...
            "
          >
            I. Total credits used to calculate your cumulative GPA
          </td>
        </tr>
        <tr v-for="(label, index) in labels" :key="index">
          <td>
            <input
              class="animation"
              :id="'credit_input' + index"
              placeholder="0"
              v-model.number="credit_input[index]"
              type="number"
              min="0"
              required
            />
      ...
      <div class="grid-columns">
        <div v-for="(label, index) in labels_gpa" :key="index">
          ...
          <input
            ...
            :id="'gpa_input' + index"
            placeholder="0"
            v-model.number="gpa_input[index]"
            type="number"
            min="0"
            required
          />
          <input
            ...
            :id="'gpa_input' + index"
            placeholder="0"
            :value="calculatedGPA[index]"
            type="number"
            min="0"
            style="border: outset; animation: credit_pal 3s infinite"
            disabled
          />
        </div>
      </div>
  ...
  data() {
    return {
      labels: [
        ...
        ':Enter TOTAL transfer and test(AP, CLEP, CPE)'
      ],
